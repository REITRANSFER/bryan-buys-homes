...
export function ZeroDistractionForm({ accentColor, serviceAreas, disqualifiedPropertyTypes, phoneHref, phoneDisplay }: Props) {
  ...
  // Some clients store PHONE_HREF with a leading "tel:" (Lori's does) — strip it
  // so we never render a doubled "tel:tel:..." href.
  const cleanHref = phoneHref.replace(/^tel:/i, "")
  // SMS body sent when user couldn't qualify but still wants help.
  const smsHref = `sms:${cleanHref}?body=${encodeURIComponent("I was unable to fill out the form but I would still like an offer.")}`
  ...
  const submit = async () => {
    ...
    try {
      ...
      const eventID = emailNorm
        ? `lori_lead_${emailNorm.slice(0, 16)}`
        : `lori_lead_anon_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`
      ...
      if (typeof window !== "undefined" && window.fbq) {
        window.fbq("track", "Lead", {
          ...
          content_category: "cash_buyer_lori",
        }, { eventID })
      }
